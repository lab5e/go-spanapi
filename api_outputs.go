...
func (r ApiRetrieveOutputStatsRequest) Execute() (*OutputStats, *http.Response, error) {
	return r.ApiService.RetrieveOutputStatsExecute(r)
}
...
func (a *OutputsApiService) RetrieveOutputStatsExecute(r ApiRetrieveOutputStatsRequest) (*OutputStats, *http.Response, error) {
	var (
		...
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *OutputStats
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "OutputsApiService.RetrieveOutputStats")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/span/collections/{collectionId}/outputs/{outputId}/stats"
	localVarPath = strings.Replace(localVarPath, "{"+"collectionId"+"}", url.PathEscape(parameterValueToString(r.collectionId, "collectionId")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"outputId"+"}", url.PathEscape(parameterValueToString(r.outputId, "outputId")), -1)
