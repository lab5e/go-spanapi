...
func (r ApiRetrieveGatewayStatsRequest) Execute() (*GatewayStats, *http.Response, error) {
	return r.ApiService.RetrieveGatewayStatsExecute(r)
}
...
func (a *GatewaysApiService) RetrieveGatewayStatsExecute(r ApiRetrieveGatewayStatsRequest) (*GatewayStats, *http.Response, error) {
	var (
		...
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *GatewayStats
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "GatewaysApiService.RetrieveGatewayStats")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/span/collections/{collectionId}/gateways/{gatewayId}/stats"
	localVarPath = strings.Replace(localVarPath, "{"+"collectionId"+"}", url.PathEscape(parameterValueToString(r.collectionId, "collectionId")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"gatewayId"+"}", url.PathEscape(parameterValueToString(r.gatewayId, "gatewayId")), -1)
